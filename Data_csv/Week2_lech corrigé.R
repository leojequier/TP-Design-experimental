...
plot(cell_count2$time, cell_count2$MixC_SCmoy_G ,cex.main=2.5, cex.axis=1.3,
...
#
legend("topleft",legend=c("SC alone in mixC","SC alone in Tol","SC in PVSC in mixC","SC in PVSC in Tol"),fill=c("black","red","blue","orange"),cex=1.7)
...
legend("topleft",legend=c("PV alone in mixC","PV alone in Tol","PV in PV+SC in mixC","PV in PV+SC in Tol"),fill=c("black","red","blue","orange"),cex=1.7)
...
plot(cell_count2$time, cell_count2$Tol_PVmoy_G+cell_count2$Tol_PVmoy_R,type = "l", cex.main=2.5,
     cex.lab=1.7, cex.axis=1.3,log = "y", main = "Total count in Toluene \n Week 2",
...
legend(x="topleft", legend = c("Tol SC alone", "Tol PV alone", "Tol PVSC"), fill=c("red", "green", "orange"), cex=1.7)


par(mai = rep(1,4), pin = c(10, 6))
plot(cell_count2$time, cell_count2$MixC_PVSCmoy_G+ cell_count2$MixC_PVSCmoy_R,type = "l",
     cex.lab=1.7, cex.main=2.5, cex.axis=1.3,
...
legend(x="topleft",legend=c("MixC SC alone", "MixC PV alone", "MixC PVSC"), fill=c( "red", "green", "orange"), cex=1.7)
...
legend(x = "bottomright",legend = c("Green (only) fluorescence in PV", "Green (only) fluorescence in PVSC"), fill = c("red", "blue"), cex=1.7)
