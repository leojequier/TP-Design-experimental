...
\begin{abstract}
In case of an environmental pollution, the microbial community of the soil will have an important role in the degradation of the pollutant. However, adaptation to the sudden change in the soil composition is difficult for most bacterial communities. But bioaugmentation, the addition of a bacteria able to degrade a pollutant in order to remove that pollutant from the environment, might also have positive side effects on the growth of the original bacterial community. Furthermore, the survival of the added bacteria and thus the efficiency of bioaugmentation depends a lot on its capacity to grow in presence of the original bacterial community. In that context, we chose to study the interaction of a bacterial community extracted from sand and two toluene degrading bacteria: \textit{Pseudomonas putida} F1  or \textit{Pseudomonas veronii} YdBTEX2 (both tagged with fluorescent mCherry protein). To do so, we grew them in batch cultures either with toluene only or with mixed carbon\footnote{Mixed Carbon media composition : L-Arginine, D-Xylose, Aspartic acid potassium salt, 2-Hydroxybenzoic acid, L-Phenylalanine, L-Serine, Gamma Hydroxy butyric acid, D-cellobiose, alpha-D-Lactose, Putrescine, Itaconic acid, alpha-D-Glucose-1-phosphate, D-Glucosaminic acid, N-acetyl-D-glucosamine, 4-Hydroxy benzoic acid, D-Mannitol, meso-erytritol, Galacturonic acid, Methyl pyruvate, Tween 20. Each component in equal concentration} substrate. We used flow cytometry at different time points to measure the growth of each population separatly. If we consider negligible the difficulty we had to distingish the two populations (see \textit{Discussion}), our results notably indicate that the bacterial commnuity grows better in toluene in presence of \textit{Pseudomonas putida} F1 than alone.


%In the case of an environmental pollution, the microbial community of the soil will have an important role in the degradation of the pollutant. However, adaptation to the sudden change in the soil composition is difficult for most bacterial communities. Bioaugmentation is a method that could be used to help them overcome the effect of pollution. It is the addition of bacteria able to degrade the pollutant into the contaminated environment. In our experiment, we chose to add \textit{Pseudomonas veronii} 1YdBTEX2 and \textit{Pseudomonas putida F1} (both tagged with fluorescent mCherry protein) to a bacterial community extracted from sand and to grow them either with toluene only or with mixed carbon substrate. As the two \textit{Pseudomonas} strains we added are able to degrade toluene, we wanted to know if the addition of one of these strain to this bacterial community would have an effect on the growth of this community.
...
Apart from SC, we used two bacterial strains : \textit{Pseudomonas veronii} 1YdBTEX2 (tagged with mCherry) and \textit{Pseudomonas putida F1}(also tagged with mCherry).However, keep in mind that SC isn't a single species of bacteria, but rather a community containing potentially many different species.
We experimented with two media: a mixed carbon medium\textsuperscript{1} and a Minimal Media 1x (MM) with only toluene as carbon source. We separated the work into two weeks, testing SC and PP during Week 1 and SC and PV during Week 2. In each experiment, we tested the 2 communities and their interaction in mixed carbon and toluene (see Fig. \ref{design_weeks}). As controls, we used SC alone in both media, to prove that the medium has an effect and compare with the interaction of species. PP(or PV) alone in both media are used to show that the medium has no effect.
\begin{figure}[H]
...
We conclude that the presence of PP affects SC differently depending on the media. In toluene, it allows for a  better growth but has no effect in the mix carbon media. 
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\label{aucppw1}
\end{figure}In addition, the effect of the media on SC is important in absence of PP. SC alone grows less in toluene than in mixed carbon.\newline
...
One can also notice that PP grew slightly less in presence of SC, \begin{figure}[H]
	\centering
	\includegraphics[width=9cm]{totcount_tol2.PNG}
...
\begin{figure}[H]
	...
	\label{aucpvw2}
\end{figure}in both media (blue and yellow compared to black and red curves/points). 
The difference in area under the curve is significant (F = 5.56, df = 1, $p = 0.046$). 
While not being as significant as our other results, it could indicate some competition between them in this type of culture. It seems plausible, knowing from previous experiments that PV grows a bit slower than PP and in this experiment some bacteria from the sand community were able to grow on toluene.
\newline
